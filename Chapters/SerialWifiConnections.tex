...
ESP-based Feathers (and many other microcontrollers) have two primary modes of communication: USB and WiFi.
...
If your laptop runs Windows, OS X (Mac) or Chrome OS and you are using one of the earlier ESP chips, you may need to install a driver to enable your computer to connect with serial USB converter on your microcontroller (this is not needed on linux machines).
...
First, a bit of background: Your ESP has two modes for its WiFi connections:
\begin{itemize}
...
Your ESP is by default configured as an \emph{Access Point}.
...
MicroPython-xxx
\end{lstlisting}
\verb|xxx| is different for every individual ESP, but if multiple microcontrollers are present these are still very similar and easily confused SSID's.
...
\begin{kaobox}[frametitle=WiFi special powers]
	One of the best features of the ESP-based boards is that the built-in WiFi can operate in both Access Point and Station mode simultaneously.
	...
	To connect to your home Access Point, you can log directly onto your ESP using its own AP mode SSID.

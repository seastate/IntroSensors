...
import sys
#
#  This module provides codes to query direct and 10x frequency
#  -divided measurements from a TSL237 light sensor. 
#  compare_tsl237_divider: returns the average periods in us from
#      both measurements, their ratio and frequencies in Hz.
#  record_tsl237_divider: takes successive measurements and
#      queries the user whether or not to record them to a file.
#
...
def compare_tsl237_divider(p_tsl237=4,p_div=5,timeout=300000, \
    ...
    for avg in range(navg):
        try:
            per_d,status_d=median_of_n(pd,nrep,timeout)
            per_t,status_t=median_of_n(pt,nrep,timeout)
            avg_t += per_t
            avg_d += per_d
            if verbose:
                print(per_t," \t  ",per_d," \t  ",status_t, \
                      " \t  ",status_d)
        except Exception as ex:
            print('error in frequency measurement, ',ex)
    avg_t/=float(navg)
    avg_d/=float(navg)
    print(avg_t," \t  ",avg_d," \t  ",avg_d/avg_t," \t  ", \
          1000000/avg_t," \t",divide*1000000./avg_d)
    sleep(0.1)
    return [avg_t,avg_d,avg_d/avg_t,1000000/avg_t, \
            divide*1000000./avg_d]
...
def record_tsl237_divider(p_tsl237=4,p_div=5,timeout=300000, \
                           nrep=7,navg=8,divide=10.,nsample=5, \
    ...
    if file_exists(filename):
        ...
        if reply[:1] == 'o':
            print("opening new file...")
             # If the first sample, overwrite old tmp_file if it exists
    ...
    while True:
        ...
        for smpl in range(nsample):
            data.append(compare_tsl237_divider(p_tsl237=p_tsl237, \
                        p_div=p_div,timeout=timeout, nrep=nrep, \
                        navg=navg,divide=divide,verbose=verbose))
